...
1. **Initialization & Ingestion**
   * `remind init [notebook name]`: Initializes a new project in the Vault, creating the base `map.index` JSON and unique project hash.
...
3. **Writing & Modifying (AI-Optimized)**
   * `remind write <logical_path> --file <path>`: Creates a new node or overwrites an existing one using content from a temporary file. The logical path does not include the file itself.
...
* **Hashes:** Only the CLI can create hashes, there is no way to create your own.